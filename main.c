...
int c, p, b, a; //Contagem de Clientes, Produtos, Buy e Auxiliar
...
struct Carrinho finalizando(struct Carrinho *car){
	int k = 0;
	system("cls");
	printf("Cliente: %s		Codigo: %s \n", car[b].client[b].nome, car[b].client[b].codClient);
	printf("==========================\n");
	printf("Produto Comprado: %s		Quantidade: %d\n", car[b].quant[k]);
	system("pause");
	
}

struct Produtos gpsProduto(struct Produtos *prodList, struct Carrinho *car) {
  int confirm = 0;
  int k = 0;
  int y = 0;
  int buying;
  ...
  scanf("%d", &buying);
  while (confirm != 1) {
    if (buying == prodList[k].codProd) {
	  if (k == 3) {
        finalizando(car);
      }
      confirm = 1;
      car[b].prod[k] = prodList[y];
      car[b].subtotal += prodList[y].valor; 
      y++;
	} else {
      k++;
      y++;	
...
contCarrinho(struct Produtos *prodList, struct Carrinho *car){
	system("cls");
	ListProd(prodList);

	for(a = 0; a < 3; a++){
		printf("Gostaria de comprar algum outro item %s (Sair[0]): ", car[b].client[b].nome);
		gpsProduto(prodList, car);
		printf("Inserir Quantidade do Produto");
  		scanf("%d", &car[b].quant[a]);
	}
...
struct Clientes gpsClient(int escolheClient, struct Clientes *listClient, struct Produtos *prodList, struct Carrinho *car) {
  ...
  while (confirm != 1) {
    if (escolheClient == listClient[j].codClient) { //Compara a entrada com os cod de client
      confirm = 1;
      car[b].client[b] = listClient[j]; // Faz a atribuicao dos valores da struct Client p/ Carrinho
      contCarrinho(prodList, car); //Call function para Compras
    } else {
      if (j >= 10) {
        printf("Cliente Inexistente"); // Ele encerra talvez, tem q testar
      }
      j++;
    }
  }
